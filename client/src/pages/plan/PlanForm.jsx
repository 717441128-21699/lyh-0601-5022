...
const PlanForm = () => {
  ...
  const totalPrice = selectedDevices.reduce((sum, item) => sum + (item.price * (item.quantity || 1)), 0)
  ...
  const handleAddDevice = (device) => {
    ...
    setSelectedDevices([...selectedDevices, { ...device, quantity: 1 }])
  ...
  const handleQuantityChange = (deviceId, value) => {
    setSelectedDevices(selectedDevices.map(d =>
      d.id === deviceId ? { ...d, quantity: value || 1 } : d
    ))
  }
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const data = {
        ...values,
        assessmentId: assessmentId || assessmentInfo?.id,
  ...
  const deviceColumns = [
    ...
    },
    {
      ...
      key: 'subtotal',
      width: 120,
      render: (_, record) => `¥${(record.price * (record.quantity || 1)).toLocaleString()}`
    },
    {
      title: '操作',
      key: 'action',
